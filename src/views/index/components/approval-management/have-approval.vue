    ...
    <vue-scroll>
      <div class="approval-info">
        ...
        >
          ...
          <el-table-column align="center" label="我的审批">
            <template slot-scope="scope">
              <span
                :class="{ yellow: scope.row.myApprovalState === 1, gray: scope.row.myApprovalState === 2, red: scope.row.myApprovalState === 3 || scope.row.myApprovalState === 4}"
              >{{ dicFormater("BASE_SCHOOL_INFO_APPROVAL_STATE", scope.row.myApprovalState) }}</span>
            </template>
          </el-table-column>
          <el-table-column align="center" label="最终审批状态">
            <template slot-scope="scope">
...
export default {
  ...
  computed: {
    periodFilter() {
      // 获取到学段数组
      if (this.currentSchool.stageTypes) {
        return [{ enumField: "全部学段", enumValue: "" }].concat(
          this.currentSchool.stageTypes.map(item => {
            const arr = this.dicFormater("SYS_BASE_STAGE_TYPE").filter(
              sonItem => {
                return sonItem.enumValue == item;
              }
            );
            return arr[0];
          })
  ...
  methods: {
    // 选择学段
    periodChange(period) {
      this.gradeArr = [{ key: "全部年级", value: "" }];
      this.classArr = [{ key: "全部班级", value: "" }];
