    ...
    <vue-scroll>
      <div class="audit-info">
        ...
        >
          ...
          <!-- 申请人 -->
          <el-form-item label-width="68px" label="申请人:" prop="applyTeacher" class="apply-teacher">
        ...
        >
          ...
          <el-table-column align="center" label="操作">
            <template slot-scope="scope">
              <router-link
                v-if="scope.row.myApprovalState === 2 && scope.row.finalApprovalState !==2"
                :to="{ path: '/main-common/view-approval-common', query: { approvalUid: scope.row.uid } }"
                class="gray"
              >查看</router-link>
              <router-link
                v-else-if="scope.row.finalApprovalState === 1 && scope.row.myApprovalState !== 2"
                :to="{ path: '/main-common/in-approval-common', query: { approvalUid: scope.row.uid } }"
                class="blue"
              >审批</router-link>
              <router-link
                v-else
        ...
        .then(res => {
          ...
          this.$parent.setCount(
            this.tableData.filter(item => {
              return (
                item.finalApprovalState === 1 && item.myApprovalState !== 2
              );
            }).length,
            1
          );
