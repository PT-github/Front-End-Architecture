<template>
  <div class="class-vision">
    <div class="class-info-content">
      <el-form :inline="true" size="small" :model="formInline" class="demo-form-inline statistics-header-label clearfix">
        ...
        <el-form-item label="班级:">
          <el-select v-model="formInline.teacherName" placeholder="请选择班级">
            <el-option v-for="(item, index) in classList" :key="index + '_sc'" :label="item.classFullName" :value="item.classId"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item>
        ...
        <div class="el-form-right fr">
          <el-form-item>
            <el-button class="icon-button" @click="onExport" :loading="isExportLoading"><i class="iconfont icon-daoru"></i><span class="mg-l">导出</span></el-button>
          </el-form-item>
          <el-form-item>
            <el-button class="icon-button" @click="printClassVision" :loading="showPrint"><i class="iconfont icon-daochu"></i><span class="mg-l">打印</span></el-button>
          </el-form-item>
        </div>
      </el-form>
      ...
      <div v-if="tableData.total" class="pagination-comp clearfix">
        <el-pagination
          background
    ...
    <el-dialog
      class="dialog-comp dialog-class-vision"
      :title="AEIntitle"
      width="920px"
      ...
      <el-table vkey='flow-secondTable' :data="detailData.list" v-loading="loadingTeacher" border style="width: 100%" class="table-comp">
        ...
        <el-table-column :show-overflow-tooltip="true" align="center" prop="lectureTeacherName" label="申请教师" ></el-table-column>
      ...
      <printClassVision ref="printClasssVisioncom" :printname="isshowPrint"></printClassVision>
  </div>
</template>

<script>
import printClassVision from '../print/print-class-vision';
import { mapGetters } from 'vuex'
import { getYearTermList, getAdjustSizeList, getAdjustSizeDetail, getClassGrade, exportCourseStatistics, deleteDownLoadFile, getTerm } from '@/views/index/store/api'
...
export default {
    ...
    getTerm({schoolId: this.formInline.schoolId}).then(res => {
      ...
      this.getYearTerm().then(() => {
        ...
        if (this.yearTermList.length > 0) {
          ...
          getClassGrade({
            'where.schoolYearId': this.formInline.schoolYearId,
            'where.schoolId': this.formInline.schoolId,
            pageSize: 0
          }).then(res => {
            if (res) {
  ...
  methods: {
    printClassVision () {
      this.$refs['printClasssVisioncom'].print()
    },
    // 导出
    onExport () {
      let _this = this;
    ...
    getAdjustSize () {
      let obj = Object.assign({}, this.pagination, {
        ...
        // 'where.termId': 32,
        'where.viewType': 2,
        'where.classId': this.formInline.teacherName ? this.formInline.teacherName : null,
        'where.startDate': this.formInline.time && this.formInline.time.length > 0 ? formatDate(this.formInline.time[0], 'yyyy-MM-dd') : null,
        'where.endDate': this.formInline.time && this.formInline.time.length > 0 ? formatDate(this.formInline.time[1], 'yyyy-MM-dd') : null,
        'where.source': 'pc'
      })
