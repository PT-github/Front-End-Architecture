<template>
  <div style="margin-top: 1000px" class="print-class-table">
    <div id="AA" style="width: 100%;">
      <p style="margin-left: 40%; font-weight: bold; font-size: 16px; margin-bottom: 10px">{{printContent.schoolYearName}}({{printContent.termName}})场地课表</p>
        ...
        <el-table-column :label="weekFormatter[index]" min-width="168" align="center" v-for="(item, index) in maxCourseWeekDay" :key="index + '_weekFormatter'">
          <template slot-scope="scope">
            <div class="sub" v-if="scope.row['v' + (index + 1)]">
              <span v-if="!showShortSubject" style="color: #000000">{{ scope.row['v' + (index + 1)].subjectName }}</span>
              <span v-if="showShortSubject" style="color: #000000">{{ scope.row['v' + (index + 1)].subjectShortName }}</span>
