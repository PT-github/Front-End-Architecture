<template>
  <div style="margin-top: 1000px" class="print-class-table">
    <div id="AA" style="width: 100%;">
      <p style="margin-left: 40%; font-weight: bold; font-size: 16px; margin-bottom: 10px">{{printContent.schoolYearName}}({{printContent.termName}})教师课表</p>
      <el-table :data="tableData3" style="width: 100%;">
        ...
        >
          <template slot-scope="scope" v-if="scope.row['week' + (index + 1)]">
            <p class="sub"  v-if="!showShortSubject" style="color: #000000">{{ scope.row['week' + (index + 1)].subjectName }}</p>
            <p class="sub"  v-if="showShortSubject" style="color: #000000">{{ scope.row['week' + (index + 1)].subjectShortName }}</p>
            <p class="name"  v-if="showClass" style="color: #000000">{{ scope.row['week' + (index + 1)].className }}</p>
            <p class="name" v-if="showShortClass" style="color: #000000">{{ scope.row['week' + (index + 1)].classShortName }}</p>
            <a class="adjust-blue-icon" v-if="scope.row['week' + (index + 1)].adjustCourseDto" @click="onViewAdjust(scope.row['week' + (index + 1)])"><span>调</span></a>
            <a class="replace-orange-icon" v-if="!scope.row['week' + (index + 1)].adjustCourseDto && scope.row['week' + (index + 1)].replaceCourseDto" @click="onView(scope.row['week' + (index + 1)])"><span>代</span></a>
