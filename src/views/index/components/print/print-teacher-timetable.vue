<template>
  <div style="margin-top: 1000px" class="print-class-table">
    <div id="AA" style="width: 100%;">
      <p style="margin-left: 40%; font-weight: bold; font-size: 16px; margin-bottom: 10px">{{printContent&&printContent.schoolYearName}}({{printContent&&printContent.termName}})任课表</p>
      <el-table :data="tableData3" style="width: 100%;">
        ...
        >
          <template slot-scope="scope">
            ...
            <p class="name" v-if="showClass" style="color: #000000">{{ scope.row[item.label].className }}</p>
            <p class="name" v-if="showShortClass" style="color: #000000">{{ scope.row[item.label].classShortName }}</p>
            <a
...
export default {
  name: "print-teacher-timetable-syllabus",
  data () {
    return {
      schoolId: null,
      tableData3: [],
