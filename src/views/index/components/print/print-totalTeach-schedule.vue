<template>
  <div class="print-whole-tableTea print-total-teachSchedule">
    <div id="print-total-teachSchedule" :style="'width:' + width + 'px;'">
      <!--根据数据分页展示-->
      <template v-for="(item,temindex) in tableDataPages">
        <!--一页-->
        <div v-if="(temindex+1) !== tableDataPages.length" :style="'height:' + height + 'px;'" :key="'temp-total-schedule1' + temindex" style="width: 100%;overflow: hidden;">
          <!--标题-->
          <p style="height: 60px;width: 100%;"></p>
          <p style="text-align: center; font-weight: bold; font-size: 30px;height: 80px;line-height: 80px;">
            {{printContent.schoolYearName}}({{printContent.termName}})总任程表
          </p>
          <!--表格-->
          <el-table :data="item" style="width: 100%;overflow: hidden;">
            <el-table-column  prop="teacherName" label="教师姓名" :width="tableTitleWidth" align="center"></el-table-column>
              ...
              <el-table-column v-for="(it, idx) in maxCourse" :label="(idx + 1) + ''" :width="tableColumnWidth" align="center" :key="idx + '_maxClassIndex_' + index">
                <template slot-scope="scope">
                  <div v-if="scope.row['sub_' + index + '_' + idx]">
                    ...
                    <p class="name">{{ scope.row['sub_' + index + '_' + idx].classShortName }}</p>
                  </div>
                  <div v-if="scope.row['sub_' + index + '_' + idx]">
        ...
        <div v-else :key="'temp-total-schedule1' + temindex" style="width: 100%;overflow: hidden;">
          <!--标题-->
          <p style="height: 60px;width: 100%;"></p>
          <p style="text-align: center; font-weight: bold; font-size: 30px;height: 80px;line-height: 80px;">
            {{printContent.schoolYearName}}({{printContent.termName}})总任程表
  ...
  export default {
    data () {
      return {
        ...
        maxTitleLength: 4, // 最大标题字数
        lineHeight: 80, // 每行的高度
        autoMargin: 120,  // 左右边距之和 根据不同的浏览器这个值会有所不同!
        autoMarginTop: 135 // 上下边距之和 根据不同的浏览器和maxCourseWeekDay与maxCourse的值不同这个值会有所不同!   124 谷歌
      }
    },
    methods: {
      getBrowserVersion(){
        ...
        if(userAgent.lastIndexOf('Edge') !== -1) {
          this.autoMarginTop = 170
        } else if (userAgent.lastIndexOf('Chrome') !== -1) {
          this.autoMarginTop = 135
        } else {
          this.autoMarginTop = 135
        }
      },
      ...
      setDataInfo(){
        ...
        */
        ...
        this.onePageNum = Math.floor((this.height - 120 - 80 - 60)/this.lineHeight) - 1;
      ...
      print () {
        ...
        this.$nextTick(() => {
          // 通过canvas生成图片
          var dom = document.getElementById('print-total-teachSchedule');
